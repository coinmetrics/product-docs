...
def _write_overlay_conf(conf_path: Path) -> None:
    ...
    a warning.
    """
    overlay = f'''"""Overlay Sphinx configuration for the GitBook markdown build.
...
SUBMODULE_SOURCE = {str(SUBMODULE_DOCS_SOURCE)!r}
SUBMODULE_ROOT = {str(SUBMODULE_ROOT)!r}
...
def post_process_file(path: Path) -> None:
    ...
    for src_rel, target_rel in PAGE_RENAMES.items():
        src_name = Path(src_rel).name
        target_name = Path(target_rel).name
        if src_name == target_name:
            continue
        # Match ``](.../<src_name>)`` and ``](.../<src_name>#anchor)`` -- the
        # negative-lookbehind keeps us from matching unrelated names that end
        # with ``src_name`` (e.g. ``foo_index.md`` when src is ``index.md``).
        pattern = re.compile(r"\]\(([^)#\s]*?)" + re.escape(src_name) + r"(?=[)#])")
        def _swap(m: re.Match[str], target_name: str = target_name) -> str:
            prefix = m.group(1)
            if prefix and not prefix.endswith(("/", ".", "")):
                # Random suffix collision (foo + bar.md). Skip.
                return m.group(0)
