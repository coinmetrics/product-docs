    ...
    arr.forEach(asset => 
      html += `
        <tr>
          ...
          <td>
            ${asset.acl.p?.length > 0 ? `<p class="Text-regular">${asset.acl.p.join(',')}</p>` : '<cm-color-icon name="x"></cm-icon>'}
          </td>
          <td>
            ${asset.acl.u?.length > 0 ? `<p class="Text-regular">${asset.acl.u.join(',')}</p>` : '<cm-color-icon name="x"></cm-icon>'}
          </td>
        </tr>
      `
    )
