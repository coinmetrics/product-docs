...
This section walks through basic usage of the API Client. If this is your first time using the client, we recommend walking through the tutorial in our [product documentation](../../tutorials-and-examples/tutorials/walkthrough_community.md). The full list of methods can be found in the [API Reference](reference/README.md).
...
`DataCollection.first_page()` fetches and returns only the first page of results as a list of dicts. It is a lightweight way to inspect a sample of the data without downloading everything.

> **Important — incompatible with `format='json_stream'`.**  
> Many endpoints now default to `format='json_stream'`, which streams the entire result set in a
> single continuous HTTP connection. The streaming format has no concept of pages, so calling
> `first_page()` on a streaming `DataCollection` will emit a `UserWarning` and return an empty
> list. Use `.to_list()` or iterate directly over the `DataCollection` instead:
>
> ```python
> # Works for any format (streaming or paginated):
> data = client.reference_data_assets().to_list()
>
> # Only works when format='json' (non-streaming):
> data = client.reference_data_assets(format="json").first_page()
> ```
