...
1. **Horizon Selection**\
   \
   For every observation, we define a target time horizon, $$T_{target}$$ (e.g., 30 days, 90 days, or 365 days).<br>
2. **Contract Identification**\
   ...
   \
   Far-Term Options: Expiration $$T_{far} > T_{target}$$<br>
3. **ATM Selection**\
   \
   ...
   \
   $$\sigma_{far}$$: The implied volatility of the closest ATM option expiring after the target.<br>
4. **Time-Weighted Interpolation**\
   ...
   \
   The weight $$w_i$$ for each option is calculated as: $$w_i = \frac{1}{\Delta t_{i}}$$<br>
5. **Final Calculation**\
   \
