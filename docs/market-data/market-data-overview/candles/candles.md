...
{% tab title="Shell" %}
```shell
curl --compressed "https://api.coinmetrics.io/v4/timeseries/pair-candles?pairs=btc-usd&limit_per_market=1&api_key=<your_key>"
```
{% endtab %}
...
```python
import requests
response = requests.get('https://api.coinmetrics.io/v4/timeseries/pair-candles?pairs=btc-usd&limit_per_market=1&api_key=<your_key>').json()
print(response)
```
{% endtab %}
...
print(
    client.get_pair_candles(
        pairs=["btc-usd"], limit_per_market=5
    ).to_dataframe()
)
```
{% endtab %}
