...
* **Snapshot timing.** Snapshots are taken at fixed 10-second (or hourly) marks, but the `time` field carries the exchange event time of the most recent update captured before the mark, so it typically falls a fraction of a second before the round mark rather than exactly on it. `collect_time` records exchange receipt time where available, and `database_time` is stamped just after the mark when the snapshot is persisted.
