...
One observation is a single executed trade. Every trade carries the common fields below. Derivatives markets (futures and options) and decentralized-exchange (DeFi) markets each add fields specific to their type. The websocket feed carries the same fields except where noted. The columns below are the response schema for `/timeseries/market-trades`.
...
See the corresponding entry on the [Market Data FAQs page](../../resources/faqs.md) for a detailed explanation.

## Related

* [Market Quotes](market-quotes.md): level-1 best bid/ask for the same markets.
* [Market Order Books](market-order-books.md): the full depth of resting orders that trades execute against.
